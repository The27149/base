...
export class EventManager {
     ...
     * @param b 
     */
    private sortListener(a: CallBackTarget, b: CallBackTarget): number {
        if (a.order > b.order) {
            return -1;
    ...
    //注册该消息 监听者
    addEventListener(eventName: string, callBack: EventManagerCallFunc, target?: any, order: number = 1): boolean {
        if (!eventName) {
        ...
        let callTarget: CallBackTarget = { callBack: callBack, target: target, order: order };
    ...
    setEventListener(eventName: string, callBack: EventManagerCallFunc, target?: any): boolean {
        ...
        let callTarget: CallBackTarget = { callBack: callBack, target: target, order: 1 };
        this._eventListeners[eventName] = [callTarget];
        return true;
    }
    ...
    raiseEvent(eventName: string, ...eventdata: any[]) {
        ...
        if (null != this._eventListeners[eventName]) {
            // 将所有回调提取出来，再调用，避免调用回调的时候操作了事件的删除（不然迭代器会出错）
            let callbackList: CallBackTarget[] = [];
            for (const iterator of this._eventListeners[eventName]) {
                callbackList.push({ callBack: iterator.callBack, target: iterator.target, order: iterator.order });
            }
